...
def test_complex_feature_with_autosort(spark_session: SparkSession) -> None:
    test_path: Path = Path(__file__).parent.joinpath("./")

    test_name = "test_complex_feature_with_autosort"
    ...
    SparkPipelineFrameworkTestRunnerV2(
        ...
        test_name=test_name,
        test_validators=[
            OutputFileValidator(related_inputs=input_file, auto_sort=True)
        ],
        logger=logger,
