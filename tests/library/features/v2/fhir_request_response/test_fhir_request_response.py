...
from library.pipeline.fhir_request_responses.v1.pipeline_fhir_request_responses_v1 import (
    PipelineFhirRequestResponsesV1,
)
...
def test_fhir_graph_mock(spark_session: SparkSession) -> None:
    """
    expect 2 $graph calls to be made to get Organization data
    """
    test_path: Path = Path(__file__).parent.joinpath("./")
    test_name = "test_fhir_graph_mock"

    mock_server_url = "http://mock-server:1080"
    mock_client = MockServerFriendlyClient(mock_server_url)
    mock_client.clear(f"/{test_name}/")

    fhir_calls = FhirRequestResponseCalls(
        fhir_calls_folder=f"{test_path}/fhir_request_responses/Organization",
    ...
    params = {
        ...
        "files_path": [
            test_path.joinpath("fhir_request_responses/Organization/1023011178.json"),
            test_path.joinpath("fhir_request_responses/Organization/1841293990.json"),
        ],
    }
    ...
    SparkPipelineFrameworkTestRunnerV2(
        ...
        helix_transformers=[PipelineFhirRequestResponsesV1],
...
def test_fhir_id_mock(spark_session: SparkSession) -> None:
    """
    expect a request to return ids from server

    test should fail if both expectations are not met
    (meaning the data was mocked but a matching request was not made)
    """
    test_path: Path = Path(__file__).parent.joinpath("./")
    test_name = "test_fhir_id_mock"
    ...
    fhir_calls = FhirRequestResponseCalls(
        fhir_calls_folder=f"{test_path}/fhir_request_responses/request_ids",
        mock_url_prefix=f"{test_name}/4_0_0/Organization",
    )
    test_validators = MockCallValidator(related_inputs=fhir_calls)
    params = {
        ...
        "files_path": [
            test_path.joinpath("fhir_request_responses/request_ids/Organization.json")
        ],
    }
